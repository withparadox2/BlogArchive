...
function searchMusicList(Page) {
	if (document.getElementById("search_Bt").disabled == false) {
		...
		var src1 = 'http://www.xiami.com/app/nineteen/search/key/' + keyWord + '/page/' + musicPage + '?random=' + new Date().getTime() + '.js&callback=getXiamiData';
...
function getXiamiData(jsonData) {
	...
	var __musicList = "";
	if (jsonData.total == 0) {
		__musicList = '<tr valign="top"><th colspan="4">抱歉，没有找到关于 <font color="red">' + decodeURIComponent(keyWord) + '</font> 的音乐。</th></tr>';
		// return false;
	} else {
		for (var i in jsonData.results) {
			__musicList += '<tr valign="top"><td scope="row" class="check-column"><input type="checkbox" name="musicInfo';
...
function getSoundFile(id) {
	var url = getPluginURL() + 'music-search/inc/getlocation.php?id=' + id;
	var mp3_URL;
	$.ajax({
		type: "GET",
		url: url,
